...
- **Windows restore `windowConcurrency` (4) at a time**, via `restoreWindow()` batched under `Promise.all`. A window is mostly idle while its pages load, so restoring them strictly in sequence cost ~13 x 8s = ~100s for a 65-tab snapshot. Batching overlaps the waits (~30s) while still capping how many pages load at once, which is what `discardThreshold` was guarding against. A window that throws is caught, counted in `result.failedWindows`, and does not abandon the windows queued behind it. The dashboard is re-focused twice, 150ms apart, because macOS can raise a new window a beat after `create` resolves.
...
- **The version in Settings > About comes from `chrome.runtime.getManifest().version`**, never a hardcoded string (it used to be `v1.0.0` in `newtab.html`). It reports the version Chrome actually *loaded*, so if it disagrees with `manifest.json` on disk, the extension has not been reloaded. Bump `manifest.json` when behaviour changes; it is the only source of truth.
...
- **Storage self-heals note-converted sites.** Exports and stores written while the conversion bug was live (pre-1.1.0) contain records with `type: 'note'` AND a `url` — impossible for a real note, which is always created with `url: ''`. Since type is now immutable, such a record would be stuck as a note forever with no UI escape. `healConvertedNotes()` in storage.js runs on every `loadData()` and `validateImport()`: it strips the `type`, moves `text` into `note`, and rebuilds a missing name from the hostname. Idempotent; heals in memory and lets the next normal save persist. Old backups can never resurrect the bug.
